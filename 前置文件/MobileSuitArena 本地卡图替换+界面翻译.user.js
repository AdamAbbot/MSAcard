// ==UserScript==
// @name         MobileSuitArena 本地卡图替换+界面翻译
// @namespace    http://tampermonkey.net/
// @version      0.0.1
// @description  本地卡图替换及界面汉化
// @author       黑客是我
...
(function() {
    ...
    const CDN_BASE_URL = "https://cdn.jsdelivr.net/gh/MobileSuitArena/images@main/"; // 原CDN基础地址
    ...
    XMLHttpRequest.prototype.open = function(method, url, async = true, user, password) {
        if (method === "GET" && isCardRequest(url)) {
            ...
            console.log(`[卡图替换] XHR拦截：${url} → 本地：${localCardUrl}`);
    ...
    window.fetch = async function(resource, options) {
        ...
        if (isCardRequest(targetUrl)) {
            ...
            console.log(`[卡图替换] Fetch拦截：${targetUrl} → 本地：${localCardUrl}`);
            const corsOptions = {
                ...options,
                mode: "cors",
                credentials: "omit"
            ...
            try {
                ...
                console.warn(`[卡图替换] 本地加载失败，回退服务器：${localCardUrl}`);
    ...
    const replaceImgCard = (imgElement) => {
        if (imgElement.src && isCardRequest(imgElement.src)) {
            ...
            if (imgElement.src !== localCardUrl) {
                console.log(`[卡图替换] IMG标签替换：${imgElement.src} → 本地：${localCardUrl}`);
                imgElement.src = localCardUrl;
                imgElement.crossOrigin = "anonymous";
                imgElement.onerror = function() {
                    console.warn(`[卡图替换] IMG加载失败，回退服务器：${localCardUrl}`);
                    // 回退时保留文件夹结构，替换为原CDN地址
    ...
    function replaceAllText(targetNode) {
        ...
        while ((node = walker.nextNode())) {
            ...
            if (text !== originalText) {
                ...
                console.log(`[汉化] 替换: ${originalText} → ${text}`); // 增加调试日志
            }
        }
    }
    ...
    function initLocalization() {
        ...
        console.log("[汉化模块] 已初始化");
    }

    function initCardReplacement() {
        initCardObserver();
        console.log("[卡图替换模块] 已初始化");
    }
    ...
    // 启动日志
    console.log("=== 🔧 MobileSuitArena 替换工具已启动 ===");
    console.log(`本地HTTP服务器地址：${LOCAL_HTTP_SERVER}`);
    console.log(`原CDN基础地址：${CDN_BASE_URL}`);

    // 验证本地服务器
    fetch(LOCAL_HTTP_SERVER, { mode: "cors" })
        .then((res) => {
            if (res.ok) console.log("✅ 本地HTTP服务器连接正常");
            else console.log("⚠️  本地HTTP服务器响应异常");
        })
        .catch((err) => {
            console.error("❌ 本地HTTP服务器未启动或不可达！", err.message);
        });
})();